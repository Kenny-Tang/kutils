...
//    public final static int EXCELSIZE = 5;
	...
	public static <T> void exportExcel(HttpServletResponse response, List<T> contentList, RowHandler<T> handler) {
		...
		try (OutputStream os = response.getOutputStream();) {
			...
			hs.setAlignment(HorizontalAlignment.CENTER);// 设置居中
			// 设置字体样式
			...
			for (int i = 0; i < title.length; i++) {
				...
				cellTitle[i].setCellType(CellType.STRING);
	...
	/**
	 * 海量数据导出，生产的文件的后缀名为.xml 双击就直接用excel 打开了，如果行数大于65536
	 * 就需要用07以上来打开了，如果用03则只能显示出65536条记录
	 * 
	 * @param response
	 * @param contentList
	 * @param handler
	...
	private static void setExcelHead(StringBuffer sb, String[] title, int total) {
		...
		sb.append("<Table ss:ExpandedColumnCount=\"" + title.length + "\" ss:ExpandedRowCount=\"" + total + "\" x:FullColumns=\"1\" x:FullRows=\"1\" ss:StyleID=\"s62\">");
		// 设置表头
